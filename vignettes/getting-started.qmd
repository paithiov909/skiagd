...
**skiagd** is a *toy wrapper* around [rust-skia](https://github.com/rust-skia/rust-skia) designed for creative coding in R. Calling it a *toy wrapper* means the package is not intended to cover the entire Skia API. The goal is to offer a compact and expressive drawing toolkit.
...
* `as_nativeraster()`: convert the picture to an R nativeRaster object,
...
Note that Skia does not fully support rotating shapes defined only by rectangles, so in this example all rectangles are unrotated to avoid errors.
