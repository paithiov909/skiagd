...
To build from source, it requires freetype, fontconfig, and the Rust toolchains.
...
1. **Create a canvas** with a background color. The call `canvas()` returns a new [picture](https://paithiov909.r-universe.dev/skiagd/doc/manual.html#pictures) object that represents the drawing state.
2. **Compose shapes** by piping calls to `add_*()` functions. Each `add_*()` call takes the previous picture, adds shapes to it, and returns a new picture. The picture object always remains a recipe of drawing commands, no pixels are produced at this stage.
...
Here we first generate coordinates for a simple rose curve and then draw it with a single call to `add_point()`. We supply the points as a matrix and specify painting attributes via `paint()`. Here we set the color to `"white"`, the line width to `3` pixels and use `PointMode$Polygon` to connect successive points. And finally, we call `draw_img()` to render the picture on the current graphics device.
...
* [Exploring Rtistry: Art Creation with R](https://zenn.dev/paithiov909/articles/skiagd-rtistry?locale=en)
