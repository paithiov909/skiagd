...
3. **Render the picture**. To convert a picture into pixels you can call...
   ...
   2. `as_nativeraster()` to obtain a nativeRaster for plotting in base R
   3. `as_png()` to obtain a PNG as a raw vector

Despite its name, skiagd presents itself as a toy wrapper and does not behave like an R graphics device. The package is meant to draw images independently of R's graphics device system.

### Paiting Attributes

Skia exposes a rich set of [painting attributes](https://rust-skia.github.io/doc/skia_safe/type.Paint.html) that control how shapes appear when drawn. In skiagd, you can specify these attributes via the `paint()` function and pass the resulting object to the `props` argument of `add_*()`.
...
Here we first generate coordinates for a simple rose curve and then draw it with a single call to `add_point()`. We supply the points as a matrix and specify painting attributes via `paint()`. Here we set the colour to `"white"`, the line width to `3` pixels and use `PointMode$Polygon` to connect successive points. And finally, we call `draw_img()` to render the picture on the current device.
...
The animation consists of multiple rotating circles whose radii and phases change over time. You don't need to understand the details of the trigonometry here, the key takeaway is that you can compute complex coordinates in R, pipe them into `add_circle()` and `add_line()`, and then assemble the frames into a GIF using [gifski](https://github.com/r-rust/gifski).
